...
- Got a permission denied error using SCP because the .pem file had wrong permissions on WSL. Fixed it by copying the .pem file into the WSL home folder and running chmod 400 on it.
- Git push was rejected because the GitHub repo already had a README and my local repo didn't share that history. Fixed it by pulling with --allow-unrelated-histories before pushing again.

## Learnings

- How to launch and configure an EC2 instance from scratch.
- How to set up a security group and open specific ports.
- How to install and manage Nginx on Ubuntu.
- How to transfer files from a local machine to a remote server using SCP.
- Basic Git workflow, including handling a rejected push against an existing remote repo.
